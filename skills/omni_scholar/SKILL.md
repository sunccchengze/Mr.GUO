...
| 能力 | 方法 | 输出 |
| :--- | :--- | :--- |
| ① 元数据与 DOI 核验 | `verify_doi(pid)` | 声明值 / 核验值 / 正文嗅探值三方比对，状态为 `一致` / `冲突` / `出版商核验`（无本地原文，DOI 来自 Crossref/出版商页面）/ `未核验` / `仅核验值` |
...
【2】DOI 核验（逐篇三方比对）
  ...
  OK P04 [出版商核验]: 声明=10.1115/1.4051416 | 核验=None | 正文=None
  OK P14 [出版商核验]: 声明=10.1115/GT2024-128792 | 核验=None | 正文=None
  OK P15 [一致]: 声明=10.1109/CEC65147.2025.11043110 | 核验=10.1109/CEC65147.2025.11043110 | 正文=...
  → 真正冲突的篇目：无（标 OK[出版商核验] 者为无本地原文、DOI 经 Crossref/出版商页面核验；标 ?! 者为既无原文也无 DOI）
