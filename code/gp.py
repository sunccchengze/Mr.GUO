...
本全集十余篇论文的方法核心都是 **Kriging / 高斯过程**：
  * 论文 01（CR-EI）、论文 02（Filter-GEI）—— 在 Kriging 上构造期望改善准则
  * 论文 03（ASMO/MFSIK）、论文 14（MFSIK 变体）—— 以 Kriging 为代理
  * 论文 04 —— **Kriging + LHS + Sobol 敏感性**做槽缝几何不确定量化
  * 论文 05 —— 均匀设计采样 + **ANOVA** 方差分解
  * 论文 16（GPR-A，又称 PCGPR）、论文 17（多保真度 GP）、论文 18（EGO/CoKriging/GP-Hedge）
