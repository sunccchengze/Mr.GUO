...
本书面向**已经读完第零章、第 01 讲和第 02 讲的大二本科生**，帮助他们逐步进入燃气轮机设计、科学机器学习与论文阅读。它把材料分成“第一遍主路”和“论文深读”两层，不再要求初学者一次读完全部技术细节。
...
第零章、第 01、02 讲按原路径阅读。**从第 03 讲开始，先读“第二阶段：从第 03 讲开始，先学会看问题”**，再按兴趣进入后面的论文深读稿。论文深读稿中的公式、偏导、实现细节和参数表不是第一遍的必修内容。
...
- **大二本科生第一遍**：第零章 → 01 → 02 → 第二阶段主路；读完每讲停在“停读检查”。  
- **有数学基础、准备精读**：再回到第 03–20 讲原稿的公式和实验细节。  
- **要做研究**：每讲 §6 软肋 = 选题富矿。  
- **要复现**：`code/README.md`。
...
| 公式 | 全书使用当前预览可直接显示的 Unicode 数学排版；不使用不会渲染的 LaTeX 定界符 |
