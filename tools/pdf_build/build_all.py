...
for folder in ['00-第零章', '01-第一篇', '02-第二篇', '03-第三篇', '04-第四篇']:
    JOBS.append((folder, '-1.md', 2))
    JOBS.append((folder, '-2.md', 2))
    JOBS.append((folder, '-3.md', 2))
JOBS.append(('05-第五篇', '-1.md', 1))   # 代码密集，单栏
JOBS.append(('06-第六篇', '-1.md', 2))
JOBS.append(('07-附录', '-1.md', 1))     # 图册，单栏大图
...
def render():
    ...
    for folder, suffix, cols in JOBS:
        md = os.path.join(OUT, folder, BASE + suffix)
        pdf = md[:-3] + '.pdf'
        assert os.path.exists(md), f'缺 md：{md}'
        r = subprocess.run([sys.executable, md2pdf, md, pdf, '--columns', str(cols),
                            '--img-cache', IMG_CACHE, '--fontdir', FONTDIR],
                           capture_output=True, text=True, cwd=ROOT)
        if r.returncode != 0:
            print(f'FAIL {folder}/{suffix}:\n{r.stderr[-3000:]}')
            sys.exit(1)
        print(f'{folder} {suffix} -> {cols}栏 OK')
...
def verify():
    ...
    print('\n%-10s %-6s %4s %8s %4s  %s' % ('文件夹', '文件', '页数', '大小', '图数', '字体'))
    total_pages, total_size = 0, 0
    for folder, suffix, _ in JOBS:
        pdf = os.path.join(OUT, folder, BASE + suffix)[:-3] + '.pdf'
        r = PdfReader(pdf)
        fonts, nimgs = set(), 0
        for pg in r.pages:
            nimgs += len(pg.images)
        ...
        print('%-10s %-6s %4d %7.1fM %4d  %s' % (
            folder, suffix, len(r.pages), size / 1048576, nimgs, fnames))
        # 预览图：每个 PDF 的第 1 页 + 大文档追加中间页
        d = fitz.open(pdf)
        picks = [0] if len(d) <= 6 else [0, len(d) // 2]
        for p in picks:
            tag = f'{folder}_{suffix[1]}_p{p + 1}'
