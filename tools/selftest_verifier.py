...
MUTATIONS = [
    ...
    ("A5 无源倍数", WP, "\n## 6.5 全书收口",
     "\n本方法把效率提升了 3 倍。\n\n## 6.5 全书收口", "A5"),
    # 第十三轮：规范 §三.1 的"CFD 次数"并入 A5（旧裁判只查 % 与倍数，次数裸奔）
    ("A5 无源次数", WP, "\n## 6.5 全书收口",
     "\n该代理共花费 7777 次 CFD。\n\n## 6.5 全书收口", "A5"),
    # 第十三轮：D8 索引文档数字可溯（五份根文档从此也有裁判）
    # 注意：必须注入成独立块——全集的要点列表整块共享一个"见上引"标注，
    # 注入在列表内部会被块级豁免掩盖（这正是块级判定要防的同族盲区）。
    ("D8 全集无源百分比", QUANJI, "\n#### 17. ",
     "\n实验测得效率提升 77.7%。\n\n#### 17. ", "D8"),
    ...
    # 目录跳转：把稳定锚点改回旧版中文 slug（历史上目录几乎全坏的根因），
    # 必须被 A10 抓到——既要拒绝"中文 slug 残留"，也要拒绝"TOC 指向不存在的 id"。
    ("A10 目录锚点回退中文 slug", WP,
     "](#lec-01)",
     "](#第01讲-cr-ei：贝叶斯优化为什么会瞎逛)", "A10"),
    ("R1 占位回译回流", LECT07, "**回译：**", "**回译：** 上式是主路径构件——先记住它在算法里负责哪一步。", "R1"),
    ("R2 乱码公式回流", LECT03, "**〔式〕**", "**〔式〕** ω = (1)/(1 + x) ", "R2"),
    ("R3 符号卡备注清空", LECT10, "| :--- | :--- | :--- |\n| x | 设计变量 | 本例 28 个 |", "| :--- | :--- | :--- |\n| x | 设计变量 |  |", "R3"),
    ("R4 前置知识块被删", LECT17, "#### 开讲前置知识检查（大二生版）", "#### 开讲杂谈", "R4"),
]
