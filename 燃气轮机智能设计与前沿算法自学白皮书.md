...
> 本书基于西安交通大学郭振东老师及其合作团队的 15 篇核心文献与 5 篇前沿发表物，融合国际四大顶级科研工具（`gpt_academic`、`STORM`、`ChatPaper`、`PaperQA2`）的认知分析模型。
> 全书坚持**“极高自学性”**与**“第一性原理”**：从初高中最经典的力学、压强、能量守恒、概率分布出发，不搞晦涩的科研名词空架子，以生动幽默、严谨推导的教学名师风格，带你一步步登顶现代叶轮机械智能设计的学术最高殿堂！
...
很多刚进入大学的同学，一看到“跨声速压气机”、“非轴对称端壁”、“纳维-斯托克斯方程”、“高斯过程回归”就会心里发怵。别慌！**科学的本质是相通的**。世界上最先进的航空发动机，其物理内核依然建立在初中和高中就学过的基础原理之上。

### 0.1 燃气轮机到底是个啥？——吸气、压缩、点火、喷气四部曲

想象你手里有一支装满空气的打气筒：
1. **吸气（进气道）**：把外界大团大团的空气拉进来。
2. **压缩（压气机 Compressor）**：初中物理告诉我们，$P \cdot V / T = \text{常数}$，你死命往下按打气筒，空气体积变小，**压强剧烈上升，温度也跟着急剧升高**。在燃气轮机里，这就是由十几排高速旋转的叶片（压气机叶栅）在极其狭窄的空间内层层接力把空气压缩到几十个大气压。
3. **点火加热（燃烧室 Combustor）**：高压高温空气进入燃烧室，喷入航空煤油点火。根据热力学第一定律，吸收大量化学能后，气体膨胀，温度飙升至 1600℃~2000℃（这温度足以熔化钢铁！）。
...
```
     【压气机】                 【燃烧室】                【涡轮】
 ┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
 │ 多级叶片压缩空气 │ ───>  │ 喷油剧烈燃烧放热 │ ───>  │ 高温高压燃气冲击 │ ───> 喷出产生推力
 │ 压强升至30+大气压│       │ 温度飙到1700℃+  │       │ 吸收能量带动主轴 │
 └─────────────────┘       └─────────────────┘       └─────────────────┘
```
...
- 作用在质点上的力包括：前后两端的**压力差**（压强梯度）、相邻流体层之间的**黏性摩擦力**（摩擦阻力）、外界重力等。
...
```
【过度探索 Over-exploration 困境】：
算法为了消除方差，频繁在贫瘠的角落浪费高昂的 CFD 算力！
                     ┌───────────────────────────┐
                     │ 探索 (Exploration)：方差大 │
                     └─────────────┬─────────────┘
                                   │ 动态权衡
...
```
【超百维优化双子星】：
1. GSDE (AST 2023)  ───>  将 RBF 代理模型注入差分进化种群变异（126维压气机叶栅，1500次CFD提升1.104%效率）
2. DA-EGO (Eng Opt) ───>  高维空间动态拆解为低维子空间，分头搜集后聚合为精英点
```
...
```
【第一代：经典代理优化与采样准则改良 (2021)】
   CR-EI (采集准则校准) + Filter-GEI (多保真并行) ──> 解决低维黑箱的过探索与高保真算力开销

【第二代：超百维黑箱维度破局与局部聚类融合 (2023-2024)】
   GSDE (RBF辅助差分进化, 126维) + DA-EGO (动态子空间聚合) + EMFS (DBSCAN聚类) ──> 攻克百维工业级压气机叶栅联合优化

【第三代：生成式模型与跨任务设计知识迁移 (2024-2026)】
   SW-VAE (样本加权隐空间) + GTO (GAN+梯度重参数化) ──> 告别“从零开始优化”，历史优秀叶片经验直接复用

【第四代：物理增强神经算子与 AI4Turbomachinery (2025-2026)】
   TNO (全景N-S场预测) + SDNO (气膜物理叠加算子) + ResUNet (相似原理宽工况) + SHAP (93维可解释挖掘) ──> 实现秒级CFD全场预测与智能可解释设计
```
...
```
                    【非轴对称端壁 (NAE) 控流物理机制】
  传统平坦端壁:     压力面(高压) ──────── 横向强压力差 ────────► 吸力面(低压)
                                (剧烈横向二次流 ➔ 形成大通道涡)

  郭老师 NAE 造型:   压力面侧【凸起】(局部降压) ── 减小横向压差 ──► 吸力面侧【凹陷】(局部增压)
                                (横向二次流被大幅削弱 ➔ 损失降低 14.0%!)
```
...
#### 选题 1：基于生成扩散模型（Diffusion Model）的三维跨声速叶栅全拓扑逆向逆设计
...
#### 5. 选题 5：基于微强化学习（RL）与合成射流自适应协同的透平端壁主动流动控制
...
> **结语：**
> 航空发动机与燃气轮机被誉为现代工业“皇冠上的明珠”，而人工智能与先进优化算法则是点亮这颗明珠的强大引擎。
> 希望本书能成为你推开科研大门最坚实的一块垫脚石。愿你在这片充满挑战与魅力的天空中，既能脚踏实地推演每一个公式，又能仰望星空勇闯学术无人区！
